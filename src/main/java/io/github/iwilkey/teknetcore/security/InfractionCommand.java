...
public class InfractionCommand implements CommandExecutor {
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		Security.inform((Player)sender, ChatColor.DARK_RED + "You have " 
		   + Security.infractionRecordOf((Player)sender).infractions + 
		   " infraction(s). If you get " + (Security.MAX_INFRACTIONS_BEFORE_KICK - 
				   Security.infractionRecordOf((Player)sender).infractions + 1) + 
		   " more infraction(s) you will be kicked and possibly banned!" + ChatColor.RESET);
		return true;
	}
}
